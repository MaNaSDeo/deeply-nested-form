  ...
  return (
    <div className="space-y-4">
      ...
      {fields.map((note, noteIndex) => {
        return (
          <div className="py-4 bg-white p-6 rounded-lg shadow-xl" key={note.id}>
            <div>
              <div className="flex justify-between">
              ...
              <label title={'Title'} className="inline-block">
                ...
                <input
                  ...
                  {...register(
                    `chapters.${chapterIndex}.notes.${noteIndex}.content`,
                  )}
                />
                <div className="text-red-600">
                  {/* Error: Chapter notes content */}
                  {
                    errors.chapters?.[chapterIndex]?.notes?.[noteIndex]?.content
                      ?.message
                  }
                </div>
              </label>
      ...
      <button
        type="button"
        className="text-gray-600 text-center w-full underline underline-offset-4 py-2"
        onClick={() => append({ content: 'Default content!' })}
      >
        add note
      </button>
    </div>
